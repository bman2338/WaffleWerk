...
#region On Page Load
    ...
    private void Page_Load()
    {
        ...
        try
        {
            ...
            {
                password = Membership.Providers["MembershipProviderAdmin"].GetPassword(username, null);
            }
        ...
        catch (MembershipPasswordException ex)
        {
            if (user.IsLockedOut)
            {
                lblCurrentPassword.Text = "<Account Locked-Out>";
            }
            else
            {
                lblCurrentPassword.Text = "<Unable to retrieve password>";
            }
        }
